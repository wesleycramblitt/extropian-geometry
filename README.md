...
- **Analytic BREP bridge (optional OCCT)** — `import_brep`/`import_brep_file` (STEP/IGES/BREP → CADModel via BRepMesh) and `export_brep_step` (watertight parts → analytic solids → one STEP file), gated by `-DEXD_GEOMETRY_ENABLE_OCCT=ON`; stub backend builds everywhere.
...
Analytic BREP (STEP/IGES/BREP) import/export is an optional module backed by
OpenCASCADE; the default build keeps a stub without OCCT:

```bash
sudo apt install libopencascade-dev
cmake -B build -G Ninja -DEXD_GEOMETRY_ENABLE_OCCT=ON
cmake --build build
```
